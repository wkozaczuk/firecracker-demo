...
echo "$(date '+%Y-%m-%d %H:%M:%S.%N') : Start" >> $logfile
...
./firecracker --api-sock "$API_SOCKET" --context '{"id": "fc-'${SB_ID}'", "jailed": false, "seccomp_level": 0, "start_time_us": 0, "start_time_cpu_us": 0}' 1>> $logfile&& echo "$(date '+%Y-%m-%d %H:%M:%S.%N') : End" 1>> $logfile &
...
echo "$(date '+%Y-%m-%d %H:%M:%S.%N') : API done" >> $logfile
